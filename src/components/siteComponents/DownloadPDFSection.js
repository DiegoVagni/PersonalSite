		...
		return (
			<div className={Flexes.FlexColumnCenter }>
				...
				<div className={Download.DownloadSection }>
					<div className={Flexes.FlexColumnCenter}>
						{this.props.links.map((link) => { return (<a key={link.from} className={Download.DownloadLink} download={link.download} href={link.from}> {Locale.GetMessages(link.local)}</a>) })}
					</div>
				</div>
			</div>
		)
